...
open class CachingPlayerItem: AVPlayerItem {
    ...
    class ResourceLoaderDelegate: NSObject, AVAssetResourceLoaderDelegate, URLSessionDelegate, URLSessionDataDelegate, URLSessionTaskDelegate {
        ...
        func resourceLoader(_ resourceLoader: AVAssetResourceLoader, shouldWaitForRenewalOfRequestedResource loadingRequest: AVAssetResourceRenewalRequest) -> Bool {
            guard let initialUrl = owner?.url else {
                fatalError("internal inconsistency")
            }
            print("reload request \(initialUrl)")
            return true
        }
        ...
        func startDataRequest(with url: URL) {
            ...
            configuration.urlCache = audioCache
            session = URLSession(configuration: configuration, delegate: self, delegateQueue: nil)
            request = URLRequest(url: url)
            if let resp = audioCache.cachedResponse(for: request!) {
        ...
        func urlSession(_ session: URLSession, task: URLSessionTask, didCompleteWithError error: Error?) {
            ...
            Task.detached(priority: .background){ [self] in
                let cachedData = CachedURLResponse(response: response!, data: mediaData!)
                audioCache.storeCachedResponse(cachedData, for: request!)
            }
            processPendingRequests()
            owner?.delegate?.playerItem?(owner!, didFinishDownloadingData: mediaData!)
        }
